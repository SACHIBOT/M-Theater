            ...
            <section class="addmovie">
                <div class="addmovie-card">
                    ...
                    <form method="POST" enctype="multipart/form-data" onsubmit="combineShowtimes(event)">
                        ...
                        <div class="form-row">
                            <button id="addnewtime" type="button" onclick="addShowtimeInput()"
                                style="width:200px;margin:2rem;">Add New Showtime</button>
                        </div>
                        ...
                        <tbody>
                            <?php foreach ($movies as $movie) : ?>
                                <tr data-movie-id="<?= $movie['movie_id'] ?>">
                                    <td><input type="text" input="title" value="<?= $movie['title'] ?>" readonly required>
                                    </td>
                                    <td><input type="date" input="startdate" value="<?= $movie['startdate'] ?>" readonly
                                            required></td>
                                    <td><input type="date" input="enddate" value="<?= $movie['enddate'] ?>" readonly
                                            required></td>
                                    <td><textarea readonly input="showtimes" required><?= $movie['showtimes'] ?></textarea>
                                    </td>
                                    <td>
                        ...
                        <tbody>
                            <?php foreach ($normal_users as $user) : ?>
                                <tr data-user-id="<?= $user['id'] ?>" data-user-status="<?= $user['status'] ?>">
                                    ...
                                    <td>
                                        <button
                                            class="toggle-status-btn"><?= $user['status'] == 'active' ? 'Deactivate' : 'Activate' ?></button>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                        ...
                        <tbody>
                            <?php foreach ($staff_members as $staff) : ?>
                                <tr data-user-id="<?= $staff['id'] ?>" data-user-status="<?= $staff['status'] ?>">
                                    ...
                                    <td>
                                        <button
                                            class="toggle-status-btn"><?= $staff['status'] == 'active' ? 'Deactivate' : 'Activate' ?></button>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelectorAll('.edit-btn').forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    row.querySelectorAll('input, textarea').forEach(input => input.removeAttribute(
                        'readonly'));
