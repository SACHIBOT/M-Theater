...
if (isset($_GET['movie'])) {
    ...
    $sql = "SELECT * FROM movies WHERE movie_id = ? AND enddate >= ?";
    ...
    $today = date('Y-m-d');
    $bindResult =  $stmt->bind_param('ss', $movieId, $today);
    if ($bindResult === false) {
...
<body>
    ...
    <div class="main-content">
        <section class="booknow split">
            <div class="booknow-card split-card">
                <h1><?php $movieDetails['title'] ?></h1>
                <?php
                echo '<div class="carousel-item card">';
                echo '<img src="assets/movie_images/' . $movieDetails['image'] . '" alt="' . $movieDetails['title'] . '">';
                echo '<h2>' . $movieDetails['title'] . '</h2>';
                echo '</div>';
                ?>
            </div>
            <div class="booknow-card split-card2">
                <h1>Book Now</h1>
